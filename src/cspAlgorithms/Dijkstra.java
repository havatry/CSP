...
public class Dijkstra {
	 ...
	 * 
	 * Function: 
	 * 		该方法，只是简单地用dijkstra算法求完后，将中间结果（前驱矩阵）返回。
	 * Details:
	 * 		在前面的dijkstra算法中，只是返回路径。但是在LaracMethodWithStoring方法中
	 * 		却需要前驱矩阵。在不改变原有dijkstra求路径的方法前提下，就单独写了该方法。
	 * 		这个方法和DijkstraOfPath方法基本上是一样的。具体参数前面的DijkstraOfPath方法。
	 * Remark: 2018年9月19日 上午10:43:25
	 */
	protected int[] PrePath(int[] Node, double[][] Edge, int start) {
		final int NumberOfNode = Node.length;
		int next = -1;// 记录选择的下一个结点
		boolean[] S = new boolean[NumberOfNode];// S集合
		int[] pre = new int[NumberOfNode];// 记录前驱节点
	 ...
	 * 
	 * Function: 
	 * 		该方法是找出满足延时阈值条件的第一条最短路径。
	 * Details:
	 * 		在Yen算法的基础上，增加了判断。判断是否当前的路径已经满足给定延时阈值的条件。如果满足则立即停机。
	 * 		当然该方法，当求到一万条路径后会停机。主要是以更快的速度求出满足要求的一条最短路径。
	 * Remark: 2018年9月19日 上午10:47:36
	 */
	public List<Integer> YenBasedDelayConstraint(int[] Node, double[][] Edge, double[][] Id, int[][] IdLink, int start,
			int end, int delayConstraint) {
		...
		while (true) {
			...
			// 核心判断
			if (SatisfyDelayConstraint(shortestPath, Id, IdLink, delayConstraint))
				return shortestPath;// 判断是否满足延时阈值
	 ...
	 * 
	 * Function: 
	 * 		该方法指在判断当前的路径是不是已经满足延时阈值约束了 
	 * Details:
	 * 		主要是加快找路径的速度。避免先找很多路径，然后再从前往后判断，直至找到最佳路径。
	 * 		这里是在找路径的同时进行判断。该方法是YenBasedDelayConstraint的辅助方法。 
	 * Remark: 2018年9月19日 上午11:14:19
	 */
	private boolean SatisfyDelayConstraint(List<Integer> path, double[][] Id, int[][] IdLink, int delayConstraint) {
		if (Common.Ptheta(path, Id, IdLink) <= delayConstraint)
			return true;
		else
			return false;
	}
